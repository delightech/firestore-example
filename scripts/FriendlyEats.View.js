...
FriendlyEats.prototype.viewList = function(filters, filter_description, db) {
  ...
  if (
    ...
    this.getAllRestaurants(renderer, db);
  }
...
FriendlyEats.prototype.viewSetup = function(db) {
  var headerEl = this.renderTemplate("header-base", {
  ...
  button.addEventListener("click", function(event) {
    ...
    that.addMockRestaurants(db).then(function() {
      that.rerender();
    });
  });
...
FriendlyEats.prototype.updateQuery = function(filters, db) {
  ...
  this.viewList(filters, query_description, db);
};

FriendlyEats.prototype.viewRestaurant = function(id, db) {
  var sectionHeaderEl;

  var that = this;

  return this.getRestaurant(id, db)
    .then(function(doc) {
      var data = doc.data();
      ...
      if (ratings.size) {
        ...
        mainEl = that.renderTemplate("no-ratings", {
          add_mock_data: function() {
            that.addMockRatings(id, db).then(function() {
              that.rerender();
            });
          },
        });
